...
public class TradeOfferService implements RestService<TradeOffer, Long> {
	...
	/**
	 * Method to process a requested trade transaction
	 *
	 * @param requesterId    id of requester
	 * @param offerdTimeslot id of offered timeslot
	 * @param wantedTimeslot id of wanted timeslot
	 *
	 ...
	 */
	@Transactional
	public TimeTable tradeTimeslots(long requesterId, long offerdTimeslot, long wantedTimeslot) throws RuntimeException {
		if (tradeService.doTrade(requesterId, offerdTimeslot, wantedTimeslot)){
			var student = studentRepository.findById(requesterId).orElseThrow();
			TimeTable updatedTimeTable = new TimeTable();
			updatedTimeTable.setTimeslots(student.getTimeslots());
